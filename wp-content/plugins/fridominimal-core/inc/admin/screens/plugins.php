...
<div class="wrap about-wrap opal-wrap">
    <h1 class="dashicons-before dashicons-admin-plugins"><?php esc_html_e('Plugins Required', 'fridominimal-core') ?></h1>
                    ...
                    <div class="theme-wrapper">
                        ...
                        <?php if ( isset( $plugin_action['update'] ) && $plugin_action['update'] ) : ?>
                            <div class="update-message notice inline notice-warning notice-alt" style="display: block!important;">
                                <p><?php printf( esc_attr__( 'New Version Available: %s', 'fridominimal-core' ), esc_attr( $plugin['version'] ) ); ?></p>
                            </div>
                        <?php endif; ?>
                        <div class="theme-id-container">
                            <h3 class="theme-name">
                                <?php if ( 'active' == $plugin_status ) : ?>
                                    <span><?php printf( esc_attr__( 'Active: %s', 'fridominimal-core' ), esc_attr( $plugin['name'] ) ); ?></span>
                                <?php else : ?>
                        ...
                        <?php if ( isset( $plugin['required'] ) && $plugin['required'] ) : ?>
                            <div class="plugin-required">
                                <?php esc_html_e( 'Required', 'fridominimal-core' ); ?>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
