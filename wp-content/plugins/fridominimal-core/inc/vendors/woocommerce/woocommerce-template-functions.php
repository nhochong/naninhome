...
if (!function_exists('osf_cart_link')) {
     ...
     * @since  1.0.0
     */
    function osf_cart_link($icon = 'opal-icon-cart', $title = '', $style = '') {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            $items = '';
            $items .= '<a data-toggle="toggle" class="cart-contents header-button" href="' . esc_url(wc_get_cart_url()) . '" title="' . __("View your shopping cart", "fridominimal-core") . '">';
            $items .= '<i class="' . $icon . '" aria-hidden="true" style="' . $style . '"></i>';
            $items .= '<span class="title">' . esc_html__($title, 'fridominimal-core') . '</span>';
            ...
            $items .= '<span class="count-text">' . wp_kses_data(_n("item", "items", WC()->cart->get_cart_contents_count(), "fridominimal-core")) . '</span>';
            $items .= '</a>';
...
if (!function_exists('osf_cart_count_text')) {
    ...
    function osf_cart_count_text() {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            return '<span class="count-text">' . wp_kses_data(_n("item", "items", WC()->cart->get_cart_contents_count(), "fridominimal-core")) . '</span>';
        }
        return '';
    }
}
...
if (!function_exists('osf_handheld_footer_bar_search')) {
    ...
    function osf_handheld_footer_bar_search() {
        echo '<a href="">' . esc_attr__('Search', 'fridominimal-core') . '</a>';
        osf_product_search();
    }
}
...
if (!function_exists('osf_handheld_footer_bar_cart_link')) {
    ...
    function osf_handheld_footer_bar_cart_link() {
        ...
        <a class="footer-cart-contents" href="<?php echo esc_url(wc_get_cart_url()); ?>"
           title="<?php esc_attr_e('View your shopping cart', 'fridominimal-core'); ?>">
...
if (!function_exists('osf_handheld_footer_bar_account_link')) {
    ...
    function osf_handheld_footer_bar_account_link() {
        echo '<a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'fridominimal-core') . '</a>';
    }
}
...
if (!function_exists('osf_woocommerce_single_product_5_wrap_start')) {
    function osf_woocommerce_single_product_5_wrap_start() {
        echo '<div class="single-style-5-wrap">';
    }
}

if (!function_exists('osf_woocommerce_single_product_5_wrap_end')) {
    function osf_woocommerce_single_product_5_wrap_end() {
        echo '</div>';
    }
}
...
if (!function_exists('osf_get_loop_product_thumbnail')) {
    function osf_get_loop_product_thumbnail($size = 'woocommerce_thumbnail', $deprecated1 = 0, $deprecated2 = 0) {
        ...
        if (count($gallery) <= 0) {
            return '<div class="product-image no-gallery">' . $product->get_image($size) . '</div>';
        }
...
if (!function_exists('osf_woocommerce_product_gallery_image')) {
    function osf_woocommerce_product_gallery_image() {
        /**
...
if (!function_exists('osf_woocommerce_product_gallery_nav')) {
    function osf_woocommerce_product_gallery_nav() {
        /**
         * @var $product WC_Product
         */
        global $product;
...
if (!function_exists('osf_woocommerce_group_action_loop_start')) {
    function osf_woocommerce_group_action_loop_start() {
        echo '<div class="group-action shop-action">';
    }
}

if (!function_exists('osf_woocommerce_group_action_loop_end')) {
    function osf_woocommerce_group_action_loop_end() {
        echo '</div>';
    }
}
...
if (!function_exists('osf_woocommerce_product_loop_price_start')) {
    function osf_woocommerce_product_loop_price_start() {
        echo '<div class="product-price">';
    }
}

if (!function_exists('osf_woocommerce_product_loop_price_end')) {
    function osf_woocommerce_product_loop_price_end() {
        echo '</div>';
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_stock')) {
    function osf_woocommerce_get_product_label_stock() {
        ...
        if ($product->get_stock_status() == 'outofstock') {
            echo '<span class="out-of-stock">' . esc_html__('Out Of Stock', 'fridominimal-core') . '</span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_set_register_text')) {
    function osf_woocommerce_set_register_text() {
        echo '<div class="user-text">' . __("Creating an account is quick and easy, and will allow you to move through our checkout quicker.", "fridominimal-core") . '</div>';
    }
}
        ...
        if ($time_sale) {
            ...
            echo '<div class="time">
                    <div class="opal-countdown clearfix"
                        ...
                        data-days="' . esc_html__("days", "fridominimal-core") . '" 
                        data-hours="' . esc_html__("hours", "fridominimal-core") . '"
                        data-minutes="' . esc_html__("mins", "fridominimal-core") . '"
                        data-seconds="' . esc_html__("secs", "fridominimal-core") . '"
                        data-Message="' . esc_html__('Expired', 'fridominimal-core') . '"
...
if (!function_exists('osf_active_filters')) {
    function osf_active_filters() {
        ...
        echo '<span class="osf_active_filters_label">' . esc_html__('Active Filters: ', 'fridominimal-core') . '</span>
    <a class="clear-all" href="' . esc_url($link_remove_all) . '">' . __('Clear Filters', 'fridominimal-core') . '</a>
</div>';
    }
}
...
function osf_single_product_social() {
	if ( get_theme_mod( 'osf_socials' ) ) {
		$template = WP_PLUGIN_DIR . '/filaminimal-core/templates/socials.php';
...
if (!function_exists('osf_woocommerce_deal_progress')) {
    function osf_woocommerce_deal_progress() {
        ...
        <div class="deal-sold">
            <span class="deal-text d-block"><span><?php esc_html_e('HURRY! ONLY', 'fridominimal-core') ?></span> <span class="c-primary"><?php echo esc_attr(trim($limit - $sold))?></span> <span><?php esc_html_e('LEFT IN STOCK.', 'fridominimal-core') ?></span></span>
            <div>
...
function otf_woocommerce_recently_viewed_product(){
	if(get_theme_mod('otf_woocommerce_extra_enable_product_recently_viewed', false)){
		...
		if(! empty( $_COOKIE['otf_woocommerce_recently_viewed'] )){
			...
			echo '<h2 class="otf-woocommerce-recently-viewed">'. esc_html__('Your Recently Viewed', 'fridominimal-core').'</h2>';
