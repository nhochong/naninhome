...
class OSF_Widget_Product_Brands extends WC_Widget{
    ...
    public function __construct() {
        ...
        $this->widget_description = __( 'A list or dropdown of product brands.', 'fridominimal-core' );
        $this->widget_id          = 'woocommerce_product_brands';
        $this->widget_name        = __( 'Product Brands', 'fridominimal-core' );
        $this->settings           = array(
            'title'  => array(
                'type'  => 'text',
                'std'   => __( 'Product Brands', 'fridominimal-core' ),
                'label' => __( 'Title', 'fridominimal-core' ),
            ),
            'dropdown' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show as dropdown', 'fridominimal-core' ),
            ),
            'show_logo' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show logo brand', 'fridominimal-core' ),
            ),
            'count' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show product counts', 'fridominimal-core' ),
            ),
            'hierarchical' => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => __( 'Show hierarchy', 'fridominimal-core' ),
            ),
            'show_children_only' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Only show children of the current brand', 'fridominimal-core' ),
            ),
            'hide_empty' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Hide empty brands', 'fridominimal-core' ),
            ),
            'max_depth'  => array(
                'type'  => 'text',
                'std'   => '',
                'label' => __( 'Maximum depth', 'fridominimal-core' ),
            ),
        );
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $dropdown ) {
            ...
            include_once( FRIDOMINIMAL_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-product-brand-list-walker.php' );

            $list_args['walker']                     = new OTF_Product_Brand_List_Walker;
            $list_args['title_li']                   = '';
            $list_args['pad_counts']                 = 1;
            $list_args['show_option_none']           = __( 'No product brands exist.', 'fridominimal-core' );
            ...
            echo '<input type="text" id="otf-product-brands-'. $id .'" onkeyup="Otf_search_product_brand();" placeholder="'. esc_attr__('Search for brand name...', 'fridominimal-core').'" title="'.esc_attr__('Type in a name','fridominimal-core').'">';
