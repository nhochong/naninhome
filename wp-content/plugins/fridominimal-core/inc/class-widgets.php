...
class OSF_Recent_Posts_Widget extends WP_Widget_Recent_Posts {
    ...
    public function widget($args, $instance) {
        ...
        $title = (!empty($instance['title'])) ? $instance['title'] : __('Recent Posts', 'fridominimal-core');
        ...
        <ul>
            <?php foreach ($r->posts as $recent_post) : ?>
                ...
                $title      = (!empty($post_title)) ? $post_title : __('(no title)', 'fridominimal-core');
                ?>
                <li>
                    <?php
                    if ($show_image) {
...
class OSF_Account_Widget extends WP_Widget {
    public function __construct() {
        $widget_ops = array(
            ...
            'description'                 => __('Display Login/Register button when click then show popup', 'fridominimal-core'),
            'customize_selective_refresh' => true,
        );
        parent::__construct('otf-widget-accounts', __('OTF Login/Register Button', 'fridominimal-core'), $widget_ops);
